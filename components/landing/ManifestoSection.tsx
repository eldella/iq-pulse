...
function ManifestoCard({
  ...
  return (
    ...
    >
      <GlassCard
        variant="plain"
        className="relative flex h-full flex-col gap-3 p-6 shadow-sm transition-shadow hover:shadow-md"
      >
        <span className="flex h-11 w-11 items-center justify-center rounded-full border border-glass-border bg-surface-hover">
          <Icon className="h-5 w-5 text-accent" aria-hidden="true" />
        </span>
        <h3 className="text-xs font-semibold uppercase tracking-[0.15em] text-accent">
          {title}
        </h3>
        <div className="flex flex-col gap-3 text-base text-pretty leading-relaxed text-muted-foreground">
          {children}
        </div>
      </GlassCard>
    </motion.div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ManifestoCard icon={Compass} title={t.manifesto.card1.title}>
            <p>{t.manifesto.card1.body}</p>
          </ManifestoCard>
        </motion.div>

        <motion.div variants={{ hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: springTransition } }}>
          <ManifestoCard icon={UserRound} title={t.manifesto.card2.title}>
            <p>{t.manifesto.card2.body}</p>
          </ManifestoCard>
        </motion.div>

        <motion.div variants={{ hidden: { opacity: 0, y: 24 }, show: { opacity: 1, y: 0, transition: springTransition } }}>
          <ManifestoCard icon={Sprout} title={t.manifesto.card3.title}>
